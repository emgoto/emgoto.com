...
The inline edit UI pattern allows users to directly edit content without having to navigate to a separate edit screen. In this tutorial, we’ll be building an accessible inline edit component in React. Here’s the final product:

\<INSERT FINAL PRODUCT HERE\>

We’ll also be taking a look at how to write some unit tests with React Testing Library. Let’s get started!
...
Let’s get started by creating an `InlineEdit` component:
...
We’re also passing in `value` and `setValue` props. In a real-world app, you’ll be getting and setting values using an endpoint. For this example, we’ll use a `useState` hook to store and set our value:
...
## Add CSS to create a “view” mode
We’ll add some CSS to remove the input styling, and make the component look as though it is in a “view” state.
...
Currently we call the `setValue` prop on each key press. We’ll want to prevent this from happening until a user exits the input. 
...
The one difference with textarea to input is that you pass in a `rows` value. This specifies the height of your textarea.
...
And finally, we can use a cool library called [jest-axe](https://github.com/nickcolley/jest-axe). You can use it to assert that your component doesn’t have any accessibility violations:
