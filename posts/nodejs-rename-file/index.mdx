...
I then add the script in my repository’s `package.json` file:
...
In the sections below, we’ll be making use of a library called [fs](https://nodejs.org/api/fs.html). Make sure to install your dependencies before you run the script!
...
(The 0th and 1st spots in the array will contain `node` and `rename` the name of your script).
