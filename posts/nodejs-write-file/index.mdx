...
const callback = (err) => {
    // If there's an error, we can do something about it here.
};
...
Now you'll have a copy of the file's contents. You can use JavaScript’s replace function to modify it. For example, we could replace “Hello” with “Goodbye”:

```js
const newContent = content.replace('Hello', 'Goodbye');
```

Finally, you would need to call `writeFile` (or `writeFileSync`) to update the file with the new contents:
...
The asynchronous version is `readFile`:
...
The callback function we pass in as the third argument will be called once the file has been read (or errors out).
...
Instead of fs readFile and writeFile, there’s also a library we can use called [replace-in-file](https://www.npmjs.com/package/replace-in-file).
